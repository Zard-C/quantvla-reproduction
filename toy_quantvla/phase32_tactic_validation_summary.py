"""Summarize Phase 32 held-out tactic validation experiments."""
...
TAG_PREFIX = os.environ.get("TAG_PREFIX", "phase32_tactic_validation_30case_v1")
CASE_LIST = os.environ.get("CASE_LIST", "")
POLICY_SEED_BASE = int(os.environ.get("POLICY_SEED_BASE", "20260702"))
OUT_JSON = RESULTS / f"{TAG_PREFIX}_summary.json"
_out_md = Path(os.environ.get("OUT_MD", "docs/phase32_tactic_validation_report_zh.md"))
OUT_MD = _out_md if _out_md.is_absolute() else ROOT / _out_md
REPORT_TITLE = os.environ.get("REPORT_TITLE", "Phase 32: Held-Out Tactic Validation v2")
...
def build_summary() -> dict[str, Any]:
    ...
    return {
        "phase": "phase32_tactic_validation",
...
def build_report(data: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {REPORT_TITLE}",
        "",
        "本报告汇总一个 held-out tactic validation set。它用 matched task/init cases 对 FP16 baseline、speed-only compile、duration fallback 和 layer x duration 组合候选进行逐 case 对比。",
        "",
        "## 实验设计",
        "",
    ...
    lines += [
        ...
        "## 实验结论",
        "",
        "- 首先看 aggregate success 和 server p50 speedup，确认速度收益是否存在。",
        "- 然后看相对 FP16 baseline 的 paired repair/regression：这是判断 closed-loop risk 的核心指标。",
        "- 再看相对 `speed_only` 的 repair/regression：这能说明保护 tactic 是真的降低风险，还是只是换了一批 trajectory basins。",
        "- 如果不同 held-out slice 的 winner 不同，说明 tactic ranking 受 task/init 分布影响；最终 tactic 不能只凭单个 held-out set 决定。",
        "",
    ]
    ...
    lines += [
        ...
        "- `combo_blocks0_3_window_0_120` 是 behavior-first candidate；`window_0_120` 是 speed-constrained candidate。",
        "- 新 held-out slice 应进入 multi-fold selector，而不是单独作为新 winner 解读。",
        "",
        "## 产物",
        "",
