...
The experiments are intentionally staged. The first track studies quantization-induced perturbations: selective W4A8 fake quantization, ATM/OHB compensation, offline action drift, and 150 matched closed-loop rollouts. This is the main reproduction-oriented quantization study. The second track uses controlled action probes and prototype acceleration-boundary probes to test the same sensitivity framework beyond fake quantization. In particular, the \texttt{torch.compile} and eager-island experiments should not be read as a quantization method. They are implementation-induced policy perturbations used to ask whether a speed-oriented execution boundary can also be amplified by closed-loop dynamics, and whether sensitivity proxies can guide which boundaries to protect.
