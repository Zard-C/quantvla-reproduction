...
\begin{enumerate}
    ...
    \item We show that a strong local tactic, policy steps 0--120, is not universal: multi-fold validation and an N1.7 CLSG-BO check change the conclusion from selecting one fixed winner to selecting behavior-first or speed-constrained tactics by held-out success, paired regressions, and latency.
\end{enumerate}
...
The experiments are intentionally staged so that each stage answers the question raised by the previous one. First, controlled action perturbations, same-observation backend-drift replay, and layer/boundary probes identify sensitive directions, rollout windows, and execution boundaries. Second, mixed eager/compiled acceleration policies test whether those maps can guide a faster implementation. Third, held-out folds and the N1.7 CLSG-BO check show why the final object should be a robust tactic-selection protocol rather than a single fixed window.
...
The sensitivity-guided acceleration search starts from a 33-case matched probe spanning tasks 4, 6, and 8 with selected initial states that include easy successes, speed-only regressions, and beneficial speed-only branches. We then validate tactic selection on two additional N1.5 30-case folds over tasks 0--9, using held-out initial states 15--17 and 18--20. For N1.7, we use LIBERO-10 tasks 0, 1, 4, 6, and 8. A 15-case warm-start slice uses initial states 27--29, a BO proposal batch reuses that slice to evaluate additional duration-window tactics, and a 15-case held-out validation uses new initial states 30--32. For paired comparisons, every mode is evaluated on the same task-initialization pairs with deterministic policy seeds. We use paired repair/regression counts, first-divergence traces, EEF drift, server p50 latency, eager-request fraction, worst-fold success, and worst-fold speedup as the primary diagnostics. We use p50 serving latency as the primary speed metric because the prototype compile experiments launch isolated server processes; mean and maximum latency include cold \texttt{torch.compile}/Inductor compilation spikes and are not treated as steady-state deployment metrics.

\section{Results}

The results are organized around CLSG-TS. Section~5.1 estimates where the closed-loop system is sensitive, using action-channel, rollout-duration, real backend-drift replay, layer-boundary, and first-divergence probes. Section~5.2 uses those probes to search acceleration tactics, instantiate a small CLSG-BO search over duration windows, and evaluate the resulting speed--risk frontier. Section~5.3 maps the evidence back to the five theoretical claims.
...
We then zoomed in on the fragile task 4 init 9 case to ask how small a perturbation can still matter. Figure~\ref{fig:perturbation-budget} sweeps the $y$-action perturbation magnitude from $10^{-6}$ to $5\times 10^{-4}$ with deterministic policy seeds. The FP16 baseline succeeds in 224 policy requests. Both full-horizon and early-window perturbations remain successful through $10^{-5}$, but fail at $3\times 10^{-5}$ and above, reaching the LIBERO horizon. Thus the observed first-flip interval is $(10^{-5},3\times 10^{-5}]$ for both windows. This should not be read as a universal numerical tolerance; it is a local basin-boundary measurement for one fragile rollout. Its deployment implication is nevertheless important: an implementation error that looks far below common local operator tolerances can be behaviorally unsafe if it is aligned with a sensitive action channel and injected before a contact-critical phase.

\begin{figure}[H]
\centering
\includegraphics[width=0.86\linewidth]{fig6_perturbation_budget.pdf}
\caption{Ultra-low controlled perturbation budget on a fragile FP16-success rollout. The same task 4 init 9 trajectory is perturbed in the $y$ action channel over the full horizon or only during policy steps 0--75. Both curves flip from success to horizon failure between $10^{-5}$ and $3\times 10^{-5}$ action magnitude.}
\label{fig:perturbation-budget}
\end{figure}

Controlled probes are useful only if they connect to real implementation residuals. We therefore ran a same-observation diagnostic on the two FP16-success cases. Along the nominal FP16 rollout, we queried both the reference policy and an accelerated tactic on the same observation, recorded the per-step backend residual $\eta_t=\pi_\varphi(s_t)-\pi_\theta(s_t)$, and then replayed scaled residual sequences in closed loop:
\begin{equation}
    a_t = a_t^{\fp} + \lambda \eta_t .
\end{equation}
This is not a production deployment path; it is a directional vulnerability probe that asks whether the actual residuals produced by a tactic are aligned with closed-loop-sensitive directions.

\begin{table}[H]
\centering
\scriptsize
\setlength{\tabcolsep}{2.5pt}
\caption{Closed-loop replay of real backend drift sequences. Residuals are measured by querying FP16 and the tactic on the same FP16-nominal observations, then replayed as $a_t=a_t^{\fp}+\lambda\eta_t$. S/F denotes success/failure and policy requests.}
\label{tab:real-backend-drift-replay}
\begin{tabular}{p{0.24\linewidth}p{0.10\linewidth}p{0.12\linewidth}p{0.14\linewidth}p{0.14\linewidth}p{0.14\linewidth}}
\toprule
Residual source & Case & Window & $\lambda=0.25$ & $\lambda=0.5$ & $\lambda=1.0$ \\
\midrule
Speed-only compile & 4:9 & Full & F991 & S221 & F991 \\
Speed-only compile & 4:9 & Early & F991 & S224 & F991 \\
Speed-only compile & 6:8 & Full & S496 & S833 & F991 \\
Speed-only compile & 6:8 & Early & F991 & F991 & F991 \\
\midrule
Blocks 0--3 + 0--120 & 4:9 & Full & S222 & S223 & S224 \\
Blocks 0--3 + 0--120 & 4:9 & Early & S224 & S224 & S224 \\
Blocks 0--3 + 0--120 & 6:8 & Full & S229 & S632 & S464 \\
Blocks 0--3 + 0--120 & 6:8 & Early & S229 & S840 & S627 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:real-backend-drift-replay} connects the controlled perturbation budget to actual backend residuals. Speed-only residual sequences can flip FP16-success rollouts, including at $0.25\times$ the measured residual sequence in several windows. The flips are not monotonic in $\lambda$; task 4 init 9 shows failure, success, then failure as the residual scale increases. This is consistent with a high-dimensional, curved trajectory-basin boundary rather than a simple one-sided tolerance. In contrast, the sensitivity-guided protected tactic remains successful for all tested windows and scales up to the unscaled residual sequence. It does not reproduce the FP16 trajectory exactly--task 6 still changes request count substantially--but it lowers outcome-level regression risk. This is the central object that CLSG-TS tries to optimize: not zero residual, but residuals whose direction, timing, and module origin are less dangerous under closed-loop sensitivity.
...
We repeated the final selection logic on a GR00T N1.7 checkpoint using a small active-search sequence over duration-window tactics. This is a restricted proof of concept for CLSG-BO, not a full backend optimizer. Phase40 uses a 15-case warm-start slice over tasks 0, 1, 4, 6, and 8 with initial states 27--29. It compares FP16, speed-only compilation, and hand-chosen request windows. Phase41 fits a lightweight RBF surrogate over duration-window parameters and evaluates one BO-selected batch on the same slice. Phase42 then validates the most important candidates on new initial states 30--32. The search space is deliberately narrow: a tactic \texttt{window\_s\_e} runs policy requests $[s,e)$ through conservative eager fallback and the rest through the compiled action head.

\begin{table}[H]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{GR00T N1.7 duration-window CLSG-BO and held-out validation. Phase40 is a warm-start slice using initial states 27--29. Phase42 is a new-initialization held-out slice using initial states 30--32. Server p50 latency is the primary speed metric; repairs and regressions are paired against FP16 on the same cases.}
\label{tab:n17-clsg-bo}
\begin{tabular}{p{0.26\linewidth}ccccc}
\toprule
Deployment point & Success & p50 latency & Speedup & Repairs & Regressions \\
\midrule
Phase40 FP16 baseline & 13/15 & 85.97 ms & 1.00$\times$ & -- & -- \\
Phase40 speed-only & 12/15 & 61.99 ms & 1.39$\times$ & 1 & 2 \\
Phase40 window 0--20 & 14/15 & 76.30 ms & 1.13$\times$ & 1 & 0 \\
Phase41 window 2--12 & 14/15 & 67.50 ms & 1.27$\times$ & 2 & 1 \\
\midrule
Phase42 FP16 baseline & 13/15 & 85.42 ms & 1.00$\times$ & -- & -- \\
Phase42 speed-only & 13/15 & 64.25 ms & 1.33$\times$ & 1 & 1 \\
Phase42 window 0--20 & 15/15 & 80.99 ms & 1.05$\times$ & 2 & 0 \\
Phase42 window 2--12 & 14/15 & 65.76 ms & 1.30$\times$ & 1 & 0 \\
Phase42 window 4--9 & 12/15 & 62.36 ms & 1.37$\times$ & 1 & 2 \\
Phase42 window 6--11 & 13/15 & 63.26 ms & 1.35$\times$ & 2 & 2 \\
\bottomrule
\end{tabular}
\end{table}

This active-search check reinforces the same lesson and makes the search protocol more explicit. In Phase40, the conservative 0--20 window is the behavior-first point: it improves success to 14/15 with no paired regressions. Phase41 then uses the warm-start observations to select additional duration windows; among them, 2--12 gives a faster speed--risk trade-off. On the new Phase42 held-out slice, 0--20 reaches 15/15 with 2 repairs and 0 regressions, while 2--12 reaches 14/15 with 1 repair, 0 regressions, and a 1.30$\times$ p50 speedup. Narrower BO proposals such as 4--9 and 6--11 are faster but reintroduce regressions. Thus BO is useful as a candidate generator, but it does not replace held-out paired validation. The correct output is a frontier: choose 0--20 when regressions are unacceptable, and choose 2--12 when a speed constraint is active.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\linewidth]{fig5_tactic_pareto.pdf}
    \caption{Speed--risk Pareto view of tactic selection. Left: N1.5 multi-fold validation shows that speed-only execution is fastest but has the largest paired regression count, while protected tactics trade speed for behavior. Right: N1.7 CLSG-BO held-out validation shows a behavior-first 0--20 tactic and a faster speed-constrained 2--12 tactic, while narrower BO proposals reintroduce regressions.}
...
Equal-norm action probes differ by channel and phase; an ultra-low $y$-perturbation sweep flips task 4 init 9 between $10^{-5}$ and $3\times 10^{-5}$ action magnitude in the full and early windows. Real speed-only backend residual replay causes multiple S$\to$F flips, while the protected residual sequence remains successful up to $1.0\times$ on the same cases. Acceleration variants with similar p50 latency differ from 2/2 success to 0/2 depending on layer boundary. In the 33-case search, duration 0--120 reaches 19/33 at 69.66 ms, while 0--220 drops to 16/33 at 90.49 ms. Across held-out folds and the N1.7 CLSG-BO check, speed-only, duration windows, combined protection, and BO proposals have different repair/regression profiles. &
...
Duration 0--120 restores FP16-level aggregate success on the probe, but does so by repairing 7 speed-only failures and regressing 4 speed-only successes. On held-out folds, speed-only is best on one slice and unsafe on another; on N1.7, \texttt{window\_0\_20} is behavior-first while CLSG-BO finds \texttt{window\_2\_12} as a faster speed-constrained point. &
The goal is not to recover the FP16 point exactly or to trust one local winner. Acceleration searches a nearby implementation space where some perturbations discover better basins and others destroy beneficial branches, so tactics and active-search proposals must be validated across slices. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item Optionally fit a lightweight surrogate over structured tactic parameters, such as fallback-window start/end, and use an acquisition rule to propose the next small batch of candidates.
...
The practical replacement is sensitivity-guided search rather than brute-force search. Cheap static and open-loop filters should first prune candidates by latency benefit, operator coverage, layerwise drift, action-channel drift, attention-routing sensitivity, and whether the candidate touches contact-critical or basin-critical regions. A smaller set can then be evaluated on proxy rollouts containing fragile matched cases, using first-divergence, EEF drift, and paired repair/regression. Only the most promising candidates should receive larger held-out closed-loop validation. In this sense, the 0--120 duration proxy is not a universal tactic; it is a candidate discovered by sensitivity-pruned search. Table~\ref{tab:phase34-multifold} shows the next step: after discovery, the tactic must be re-scored across validation folds, where behavior-first and speed-constrained objectives may choose different points on the Pareto frontier. Table~\ref{tab:n17-clsg-bo} further shows that a lightweight BO loop can propose new duration-window candidates on a newer checkpoint, but that these proposals still require held-out paired validation. The reusable object is therefore the search protocol, not the literal 0--120 or 0--20 window.

\paragraph{Active tactic search and routing.}
The N1.7 experiment suggests a design implication: a deployment need not rely on one manually chosen acceleration tactic. In our restricted pilot, BO searches only over request-window start and end. The same protocol can in principle search layer islands, precision profiles, compile targets, or conditional tactic routers. This is reminiscent of mixture-of-experts routing, but the routed objects are not model experts or learned parameters. The selector chooses inference tactics such as eager execution, speed-only compiled execution, or sensitivity-preserving fallback windows. In this view, acceleration becomes closed-loop-aware inference scheduling. The held-out result also shows the risk: a surrogate-selected or probe-selected tactic can overfit to initial states, so any search proposal or router must be evaluated by paired held-out rollouts rather than assumed safe.

This selection should be coarse-grained in a production runtime. Task-level tactic choice, episode-level profiles, or a small number of fixed phase windows can be implemented by pre-building and pre-warming a few execution profiles. In contrast, per-step, per-layer, or per-operator tactic switching can destroy the assumptions that make TensorRT engines, CUDA Graph replay, and fused kernels fast: stable shapes, stable graph structure, persistent execution contexts, and predictable memory allocation. Thus a sensitivity map should usually decide which profile to use or which broad phase to protect, not trigger arbitrary fine-grained graph changes at runtime.
...
The empirical scale is also limited. Results are centered on one GR00T N1.5 checkpoint and LIBERO-10, plus a small N1.7 active-search and held-out transfer check. The N1.5 acceleration track uses a 33-case discovery probe and two 30-case held-out folds. The N1.7 sequence uses 15-case warm-start and BO-proposal slices plus a 15-case held-out validation over tasks 0, 1, 4, 6, and 8. These experiments expose tactic instability and speed--risk structure, but they are not a definitive deployment benchmark or open-world robustness claim. The current BO instantiation searches only duration-window tactics; extending it to layer boundaries, precision profiles, production runtime tactics, online sensitivity-triggered routing, broader model families, real-robot experiments, and more exhaustive geometric diagnostics remains future work.
