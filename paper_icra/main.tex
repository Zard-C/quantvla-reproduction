...
\begin{enumerate}
    ...
    \item We demonstrate sensitivity-guided acceleration: a narrow early eager window, policy steps 0--120, restores FP16-level success on a 33-case matched set while preserving speed-only latency.
\end{enumerate}

The contribution is not a single deployment backend. Instead, we use quantization and compilation probes to expose the geometry of the problem: which perturbations are absorbed, which are amplified, which rollouts are repaired or regressed, and which dimensions, phases, or modules should be protected. The resulting design rule is simple: identify closed-loop-sensitive dimensions, durations, and modules, then protect the smallest regions that improve paired repair/regression while preserving speed.
...
The quantization analysis evaluates 150 episodes: 10 LIBERO-10 tasks and 15 initial states per task. The sensitivity-guided acceleration search uses a 33-case held-out matched set spanning tasks 4, 6, and 8, including easy successes, speed-only regressions, and beneficial speed-only branches. Every mode is evaluated on the same task-initialization pairs with deterministic policy seeds. We report success, paired repair/regression, server p50 latency, eager-request fraction, and first-divergence diagnostics.
...
The key result is not that more eager execution is safer. Speed-only compilation is fast but loses three successes relative to FP16. A broad early duration fallback, steps 0--250, recovers two successes but costs latency. The finer sweep reveals a better point: protecting only steps 0--120 restores FP16-level success, 19/33, while preserving speed-only latency. Longer windows are worse, so the sensitive region is narrow and early.
