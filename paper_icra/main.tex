...
\begin{figure}[H]
    ...
    \caption{Speed--risk Pareto view of held-out tactic selection. Speed-only is fastest but has the largest paired regression count. The combined tactic is behavior-conservative but gives up speed. The 0--120 duration tactic is the speed-constrained compromise.}
...
\begin{enumerate}[leftmargin=1.15em,itemsep=0.15em,topsep=0.25em]
    ...
    \item Re-score the frontier on held-out folds. For behavior-first selection, maximize worst-fold success and break ties by minimizing regressions. For speed-constrained selection, maximize speedup subject to explicit behavior constraints on worst-fold success and regressions.
\end{enumerate}
\emph{Output:} a behavior-first tactic or a speed-constrained Pareto tactic, with its matched success, regression, and latency report.
\end{minipage}}
\end{figure}
