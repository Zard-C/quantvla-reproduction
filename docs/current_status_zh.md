...
7. 当前最好工程 incumbent 是 `speed-only compile`。在 Phase30 held-out set 上它达到 `25/30`，p50 `50.72 ms`，相对 FP16 baseline 的 `23/30` 和 p50 `88.81 ms` 同时提升成功率和速度。
...
Phase30 使用新的 init set `15/16/17` 作为 held-out sanity set，不参与 Phase29 的 `0-120` 选择。结果显示 `0-120` 没有泛化，而 `speed-only compile` 成为当前最强 incumbent。
